...
class Calculator extends Component {
  ...
  isNaNorInfinity = () => {
    return this.state.result === "На ноль делить нельзя"
      ? true
      : this.state.result === "Результат не определен"
      ? true
      : false;
  };

  havesign = () => {
    return this.state.result.charAt(this.state.result.length - 1) in
      this.operations
      ? true
      : false;
  };
  ...
  writeNum = item => {
    if (item in this.operations) {
      this.setState({
        result: this.havesign()
          ? this.state.result
          : this.isNaNorInfinity()
          ? "0" + item
  ...
  calculation = () => {
    if (!this.isNaNorInfinity()) {
      ...
      this.setState({
        result: this.havesign()
          ? this.state.result
          : !isFinite(eval(this.state.result))
  ...
  radical = () => {
    if (!this.isNaNorInfinity()) {
      ...
      this.setState({
        result: this.havesign()
          ? this.state.result
          : !isFinite(Math.sqrt(eval(this.state.result)))
      ...
      <div className="app">
        ...
        <div className="calculator">
          <div className="top-nav">
            <Link to="/">
              <Button props={{ value: "Назад", classes: "common-button" }} />
            </Link>
