...
export default function usersReducer(state = initialState, action) {
  let arr = state.users;
  const currentUser = state.users[state.users.findIndex((el) => el.id === state.currentId)];
    ...
    case 'EDIT_USER': {
      arr = arr.map((el) => {
        if (el.id === action.userId) {
          el.user = localStorage.getItem('enterValue') === null
            ? el.user
    ...
    case 'ADD_NEW_BUTTON': {
      if (isNumeric(action.number)) {
        currentUser.button.push({
          value: String(parseInt(action.number)),
          item: String(parseInt(action.number)),
          classes: 'number',
        });
